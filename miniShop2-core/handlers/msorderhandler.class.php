...
class msOrderHandler implements msOrderInterface
{
    ...
    {
		$this->modx->log(xPDO::LOG_LEVEL_ERROR, print_r($_SESSION['minishop2'], 1), array(
			'target' => 'FILE',
			'options' => array(
				'filename' => 'orders.log'
			)
        ...
        $delivery_cost = $all_cost['data']['delivery_cost'];
        $cart_cost = $all_cost['data']['cart_cost'];
        $num = $this->getNum();
        ...
        if ($msOrder->save()) {
            ...
            if ($payment) {
                ...
                if ($this->config['json_response']) {
                    ...
                    return $this->success('', array('msorder' => $msOrder->get('id'), "order" => $ord));
                }
                $redirect = $this->modx->context->makeUrl(
                    $this->modx->resource->id,
                    array('msorder' => $msOrder->get('id'), "order" => $ord)
                );
                $this->modx->sendRedirect($redirect);
            }
            return $this->success();
    ...
    public function getCost($with_cart = true, $only_cost = false)
    {
        ...
        if (
			...
			$this->modx->log(xPDO::LOG_LEVEL_ERROR, print_r($delivery->toArray(), 1), array(
				'target' => 'FILE',
				'options' => array(
					'filename' => 'orders.log'
				)
			...
			$this->modx->log(xPDO::LOG_LEVEL_ERROR, print_r($cart, 1), array(
				'target' => 'FILE',
				'options' => array(
					'filename' => 'orders.log'
				)
			));
			$this->modx->log(xPDO::LOG_LEVEL_ERROR, print_r($delivery_cost, 1), array(
				'target' => 'FILE',
				'options' => array(
					'filename' => 'orders.log'
				)
        ...
        return $only_cost
            ...
            : $this->success('', array(
				...
				'bonus_apply' => (float)$this->order['bonus-apply'],
                'delivery_cost' => $delivery_cost
            ));
    }
