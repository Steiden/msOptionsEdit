        ...
        if ($product) {
            ...
            */
            ...
            if (array_key_exists($key, $this->cart)) {
				...
				$data = $this->status(array('key' => $key));
				$data['row'] = $cartItem;
                return $this->success(
    ...
    public function remove($key)
    {
        if (array_key_exists($key, $this->cart)) {
			...
			foreach($cart as $k => $item){
				if($k == $key){
					$cartItem = $item;
					$object = $this->modx->getObject("modResource", $item['id']);
					if($object){
						$cartItem["name"] = $object->get("pagetitle");
					}
				}
			}
            ...
            return $this->success('ms2_cart_remove_success', $data);
        } else {
